;;; -*- lexical-binding: t -*-
(custom-set-variables
 ;; custom-set-variables was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(package-selected-packages
   '(avy cape centaur-tabs colorful-mode corfu-terminal dashboard dirvish
		 doom-modeline doom-themes eat elcord eldoc-box elisp-autofmt
		 embark-consult evil-args evil-collection evil-escape
		 evil-nerd-commenter evil-surround flycheck general go-mode
		 hide-mode-line json-mode lyrics-fetcher magit marginalia
		 nerd-icons-completion nerd-icons-corfu nerd-icons-dired
		 nerd-icons-ibuffer no-littering orderless org-modern org-roam
		 projectile qml-mode quickrun rainbow-delimiters rust-mode
		 spacious-padding sublimity treesit-fold undo-fu vertico vterm
		 wakatime-mode wgrep yaml-mode yasnippet-snippets)))
(custom-set-faces
 ;; custom-set-faces was added by Custom.
 ;; If you edit it by hand, you could mess it up, so be careful.
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(emms-browser-album-face ((t (:inherit font-lock-type-face))))
 '(emms-browser-artist-face ((t (:inherit font-lock-function-name-face :weight bold))))
 '(emms-browser-composer-face ((t (:inherit font-lock-variable-name-face))))
 '(emms-browser-track-face ((t (:inherit default))))
 '(emms-browser-track-number-face ((t (:inherit line-number))))
 '(emms-browser-year-face ((t (:inherit font-lock-comment-face))))
 '(emms-playlist-selected-face ((t (:inherit highlight :weight bold))))
 '(emms-playlist-track-face ((t (:inherit default))))
 '(treemacs-root-face ((t (:height 1.0 :weight bold)))))
